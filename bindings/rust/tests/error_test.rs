...
fn test_invalid_python_exe() {
    ...
    manifest_dir.pop(); // bindings
    manifest_dir.pop(); // root
    let plugins_dir = manifest_dir.join("bindings").join("test_assets").join("plugins");
...
fn test_worker_crash_handling() {
    ...
    let dummy_script = manifest_dir.join("bindings").join("test_assets").join("crashing_worker.py");
